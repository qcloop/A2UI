...
ROLE_DESCRIPTION = (
    "You are a helpful flight finding assistant. Your final output MUST be a a2ui"
    " UI JSON response."
)
...
def get_text_prompt() -> str:
  ...
  """
  return """
    You are a helpful flight finding assistant. Your final output MUST be a text response.

    To generate the response, you MUST follow these rules:
    1.  **For finding flights:**
        a. You MUST call the `get_flights` tool. Extract the origin, destination, departure_date, optional return_date, passengers, cabin_class, and a specific number (`count`) of flights from the user's query.
    ...
    2.  **For booking a flight (when you receive a query like 'USER_WANTS_TO_BOOK...'):**
        a. Respond by asking the user for the necessary details to make a booking (passenger count, contact email, contact phone, seat preference).
...
if __name__ == "__main__":
  ...
  # For a flight booking agent, we use flight-specific examples.
  flight_prompt = A2uiSchemaManager(
      "0.8",
