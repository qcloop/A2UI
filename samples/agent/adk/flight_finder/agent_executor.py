...
class FlightAgentExecutor(AgentExecutor):
  """Flight AgentExecutor Example."""

  def __init__(self, ui_agent: FlightAgent, text_agent: FlightAgent):
  ...
  async def execute(
    ...
    if ui_event_part:
      ...
      if action == "book_flight":
        flight_number = ctx.get("flightNumber", "Unknown Flight")
        airline = ctx.get("airline", "Unknown Airline")
        depart_time = ctx.get("departTime", "Unknown Time")
        arrive_time = ctx.get("arriveTime", "Unknown Time")
        price = ctx.get("price", "Unknown Price")
        booking_link = ctx.get("bookingLink", "")
        query = (
            f"USER_WANTS_TO_BOOK: {flight_number}, Airline: {airline}, "
            f"Depart: {depart_time}, Arrive: {arrive_time}, Price: {price}, "
            f"BookingLink: {booking_link}"
        )

      elif action == "submit_booking":
        flight_number = ctx.get("flightNumber", "Unknown Flight")
        passengers = ctx.get("passengers", "Unknown")
        contact_email = ctx.get("contactEmail", "Unknown Email")
        contact_phone = ctx.get("contactPhone", "Unknown Phone")
        seat_preference = ctx.get("seatPreference", "None")
        query = (
            f"User submitted a booking for flight {flight_number} for {passengers} "
            f"passenger(s) with contact: {contact_email}, phone: {contact_phone}, "
